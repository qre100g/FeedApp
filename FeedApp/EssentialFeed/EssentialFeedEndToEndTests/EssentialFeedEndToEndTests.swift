...
//
//  Created by Mukesh Kondreddy on 16/06/26.
//
...
class EssentialFeedEndToEndTests: XCTestCase {
    ...
    func test_endToEndTestServerGETFeedImageDataResult_matchesFixedTestAccountData() {
        ...
        case let .failure(error):
            XCTFail("Expected success image data result, got \(error) instead")
        default:
    ...
    func getFeedResult(file: StaticString = #filePath, line: UInt = #line) -> FeedLoader.Result? {
        let sut = RemoteFeedLoader(url: feedTestServerURL, client: ephemeralClient())
